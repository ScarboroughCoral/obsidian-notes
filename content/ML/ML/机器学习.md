...
- Minkowski 距离
$$
L_\lambda = d(i,j)=(\sum{\vert{x_k(i)-x_k(j)}\vert}^\lambda)^{\frac{1}{\lambda}}
$$

- 欧氏距离 = $L_2$
- 曼哈顿距离 = $L_1$ 。街区路径距离
- 切比雪夫距离 = $L_{\infty} = max_k|x_k(i)-x_k(j)|$。 
- 加权欧式距离
- Bray Curtis 距离。
- 堪培拉距离。Bray Curtis距离/k
...
- KNN k的选择。哪个更好？
	- 多数情况下k=3
	- 取决于训练样例数据。更大的k不一定带来好的效果
## 效率

- KD-tree：k维度的树
	- 每次计算是$O(log_k(n))$
	- 节点记录边界left、top、right、bottom

## Lazy Learner vs Eager Learner

- lazy 基于实例，来了实例再算，不训练
- 线性回归，提前算好参数